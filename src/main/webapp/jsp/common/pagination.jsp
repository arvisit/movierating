    ...
    <c:if test="${requestScope.totalPages != 1}">
        <c:choose>
            <c:when test="${requestScope.currentPage != 1}">
                <a href="controller?command=${requestScope.paginatedCommand}&page=1">&laquo;</a>
                <a href="controller?command=${requestScope.paginatedCommand}&page=${requestScope.currentPage - 1}">&lsaquo;</a>
            </c:when>
            <c:otherwise>
        ...
        <c:choose>
            <c:when test="${requestScope.currentPage != requestScope.totalPages}">
                ...
                <a href="controller?command=${requestScope.paginatedCommand}&page=${requestScope.totalPages}">&raquo;</a>
            </c:when>
            <c:otherwise>
